...
# root_dir = str(Path(__file__).resolve().parent) + "/MLModels/"
...
with open(os.path.join('MLModels', 'reviewModel.pkl'), 'rb') as f:
    model = pickle.load(f)
# with open('MLModels\\' + 'reviewVecotorizer.pkl', 'rb') as f:
with open(os.path.join('MLModels', 'reviewVecotorizer.pkl'), 'rb') as f:
    vectorizer = pickle.load(f)


# LOAD THE MODEL AND VECTORIZER for phishing
# phish_model = open('MLModels\\' + 'phishing.pkl','rb')
phish_model = open(os.path.join('MLModels', 'phishing.pkl'), 'rb')
